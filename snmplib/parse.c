#ifndef lint
static char rcsid[]="$Header: /nfs/medea/u0/rel5/rcs/Tools/cmusnmp/snmplib/parse.c,v 1.4 1996/11/14 15:29:34 tpt2 Exp $";
#endif
...
struct tok tokens[] = {
	...
	{ "DisplayString", sizeof ("DisplayString")-1, OCTETSTR },
	{ "PhysAddress", sizeof ("PhysAddress")-1, OCTETSTR },
	{ "OF", sizeof ("OF")-1, OF },
...
hash_init()
{
	...
	/* STT Issue: bzero   */ 
	(void)memset((void *)buckets, '\0', (size_t)sizeof(buckets));
     ...
     struct node *nodes;
{
     ...
     /* STT Issue: bzero   */ 
     (void)memset((void *)nbuckets, '\0', (size_t)sizeof(nbuckets));
     for(np = nodes; np;){
         nextp = np->next;
         hash = 0;
    ...
    struct node **nodes;
{
	...
	strcpy(tp->description, np->description);
...
/*	if (tp->type == TYPE_OTHER) */
	    do_subtree(tp, nodes);	/* recurse on this child if it isn't an end
node */
    }
    ...
    char *name;
{
	...
	(void)memset((void *)np, '\0', (size_t)sizeof(struct node));
	/*
	...
	for(count = 0, op = oid, nop=oid+1; count < (length - 2); count++,
		...
		/* STT Issue: bzero   */ 
		memset((void *)np->next, '\0', (size_t)sizeof(struct node));
		oldnp = np;
		np = np->next;
	    }
	}
    ...
    char *name;
{
    ...
    np->next = 0;
    np->enums = 0;
    np->description[0] = '\0';	/* default to an empty description */
    type = get_token(fp, token);
...
type != RECOMMENDED && type != DEPRECATED){
...
#endif
	    strcpy(np->description, quoted_string_buffer);
	    break;
    ...
    register char *token;
{
    ...
    *cp = 0;
    ...
    /*
     * Accumulate characters until end of token is found.  Then attempt
to match this
     * token as a reserved word.  If a match is found, return the type.
 Else it is
     * a label.
     */
    do {
...
{
    register int ch;
    register char *cp = quoted_string_buffer;
    int eat_space = 0;
    ...
    *token = '\0';			/* make the token empty */
	...
	/* do not overflow the string */
	if (cp - quoted_string_buffer < MAXQUOTESTR - 1)
	    *cp++ = ch;
    }
