#ifndef lint
static char rcsid[]="$Header: /nfs/medea/u0/rel5/rcs/Tools/cmusnmp/snmplib/snmp_api.c,v 1.4 1996/11/14 15:29:44 tpt2 Exp $";
#endif
    ...
    struct snmp_session *session;
{
    ...
    /* STT Issue: bzero   */ 
    memset((void *)isp, '\0', (size_t)sizeof(struct snmp_internal_session));
    slp->internal->sd = -1; /* mark it not set */
    slp->session = (struct snmp_session *)malloc(sizeof(struct snmp_session));
    /* STT Issue: bcopy(3/3B) -> memcpy(3/3C) or memmove(NA/3C)
    ...
    int			*out_length;
{
	...
	return -1;
    /* STT Issue: bcopy(3/3B) */
    memmove((char *)cp, (char *)packet, totallength);
    totallength += cp - buf;
	...
	return -1;
    /* STT Issue: bcopy(3/3B) */
    memmove((char *)cp, (char *)buf, totallength);
    totallength += cp - packet;
    ...
    int	    length;
{
	...
	if (data == NULL)
	    return -1;
	data = asn_parse_int(data, &length, &type, &pdu->errstat, sizeof(pdu->errstat));
	if (data == NULL)
	    return -1;
	data = asn_parse_int(data, &length, &type, &pdu->errindex, sizeof(pdu->errindex));
	if (data == NULL)
	    return -1;
    } else {
	...
	/* STT Issue: bcopy(3/3B) 
	bcopy((char *)pdu->enterprise, (char *)objid,
	      pdu->enterprise_length * sizeof(oid));*/
	(void)memmove((char *)objid, (char *)pdu->enterprise,
		      pdu->enterprise_length * sizeof(oid)); 
	...
	if (data == NULL)
	    return -1;
	data = asn_parse_unsigned_int(data, &length, &type, &pdu->time, sizeof(pdu->time));
	if (data == NULL)
	    return -1;
    }
	...
	/* STT Issue: bcopy(3/3B) */
	memmove((char *)op, (char *)objid, vp->name_length * sizeof(oid));
	vp->name = op;
	...
	switch((short)vp->type){
	    ...
	    case GAUGE:
	    case TIMETICKS:
		vp->val.integer = (long *)malloc(sizeof(u_long));
		vp->val_len = sizeof(u_long);
		asn_parse_unsigned_int(var_val, &len, &vp->type, (u_long *)vp->val.integer, sizeof(vp->val.integer));
		break;
	    case ASN_OCTET_STR:
	    case IPADDRESS:
	    case OPAQUE:
    ...
    struct snmp_pdu	*pdu;
{
	...
	printf("sending %d bytes to %s:\n", length, inet_ntoa(pdu->address.sin_addr));
        count = 0;
        while(count < length){
            row = 0;
            for(;count + row < length && row < 16; row++){
                printf("%02X ", packet[count + row]);
	    }
            while(row++ < 16)
	      printf("   ");
            printf("  ");
            row = 0;
            for(;count + row < length && row < 16; row++){
                if (isprint(packet[count + row]))
                    printf("%c", packet[count + row]);
		else
		    printf(".");
	    }
	    printf("\n");
...
snmp_timeout(){
	...
	for(rp = isp->requests; rp; rp = rp->next_request){
		...
		if (rp->retries >= sp->retries){
			...
			printf("sending %d bytes to %s:\n", length, inet_ntoa(rp->pdu->address.sin_addr));
			count = 0;
			while(count < length){
			  row = 0;
			  for(;count + row < length && row < 16; row++){
