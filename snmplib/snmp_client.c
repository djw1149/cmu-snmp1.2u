#ifndef lint
static char rcsid[]="$Header: /nfs/medea/u0/rel5/rcs/Tools/cmusnmp/snmplib/snmp_client.c,v 1.6 1996/11/14 15:29:46 tpt2 Exp $";
#endif
    ...
    int command;
{
    ...
    /* STT Issue: bzero   */ 
    (void)memset((void *)pdu, '\0', (size_t)sizeof(struct snmp_pdu));
    pdu->command = command;
    ...
    void *magic;
{
	...
	if (var != NULL){
		...
		if (var->val.string != NULL){
		    newvar->val.string = (u_char *)malloc(var->val_len);
		    /* STT Issue: bcopy(3/3B) */
		    memmove((char *)newvar->val.string,
    ...
    struct snmp_pdu **response;
{
	...
	*response = NULL;
    ...
    /*
     * If the request has timed out then do not return a response.
     * If you do (as the code used to do) then the PDU memory will be 
     * freed twice.
     */
    if (state->status != STAT_TIMEOUT)
	*response = state->pdu;
    else 
	*response = (struct snmp_pdu *)NULL;
    return state->status;
}
